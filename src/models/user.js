...
const userSchema = new mongoose.Schema({
    ...
    email: {
        ...
        validate(value) {
            if (!validator.isEmail(value)) throw new Error('That is not a valid email address')
        }
    },
    password: {
        type: String,
        ...
        trim: true,
        validate(value) {
            if (value.toLowerCase() === 'password') throw new Error('Password cannot be used as a password')
        }
    },
    role: {
        type: String,
